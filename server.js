...
// Defaults used whenever a user's `settings` jsonb column is null/missing —
// keeps the frontend from having to guard against undefined nested fields.
const DEFAULT_SETTINGS = {
  notifications: { newMatches: true, newMessages: true, likes: true, email: true },
  privacy: { showOnlineStatus: true, showDistance: true },
  appearance: { darkMode: false },
};

function mergeSettings(base, patch) {
  const out = JSON.parse(JSON.stringify(base || DEFAULT_SETTINGS));
  for (const section of Object.keys(patch || {})) {
    if (out[section] && typeof out[section] === 'object' && !Array.isArray(out[section])) {
      Object.assign(out[section], patch[section]);
    } else {
...
function toOwnUser(row) {
  return {
    ...
    settings: mergeSettings(DEFAULT_SETTINGS, row.settings || {}),
  };
}
...
// reasons they'd get along, without ever exposing either person's raw
...
// Settings are stored as a single jsonb column so new toggles don't need a
// migration each time. Requires a `settings jsonb` column on `users` —
// add it in Supabase if it doesn't exist yet: `alter table users add column settings jsonb;`
//
// Only booleans under these known sections/keys are accepted — this keeps
// the settings blob predictable and stops a client from writing arbitrary
// keys into it.
const SETTINGS_SCHEMA = {
  notifications: ['newMatches', 'newMessages', 'likes', 'email'],
  privacy: ['showOnlineStatus', 'showDistance'],
  appearance: ['darkMode'],
};
function sanitizeSettingsPatch(body) {
  const clean = {};
  for (const section of Object.keys(SETTINGS_SCHEMA)) {
    if (!body || typeof body[section] !== 'object' || body[section] === null) continue;
    const allowedKeys = SETTINGS_SCHEMA[section];
    const sectionPatch = {};
    for (const key of allowedKeys) {
      if (typeof body[section][key] === 'boolean') sectionPatch[key] = body[section][key];
    }
    if (Object.keys(sectionPatch).length) clean[section] = sectionPatch;
  }
  return clean;
}
async function handleUpdateSettings(req, res) {
  ...
  const merged = mergeSettings(mergeSettings(DEFAULT_SETTINGS, user.settings || {}), patch);

  const { data: updated, error } = await supabase
    .from('users').update({ settings: merged }).eq('id', user.id).select().single();
  if (error) throw error;

  sendJSON(res, 200, { settings: mergeSettings(DEFAULT_SETTINGS, updated.settings || {}) });
}
...
async function handleChangePassword(req, res) {
  ...
  const { error } = await supabase
    .from('users').update({ password_salt: salt, password_hash: hash }).eq('id', user.id);
  if (error) throw error;
...
async function handleDeleteAccount(req, res) {
  ...
  // Clean up everything that references this user so deleting an account
  // doesn't leave orphaned rows behind — stale matches/likes the other
  // person would still see, or messages nobody can load anymore.
  const { data: ownMatches, error: matchLookupErr } = await supabase
    .from('matches').select('key').or(`user_id_1.eq.${user.id},user_id_2.eq.${user.id}`);
  if (matchLookupErr) throw matchLookupErr;
  const matchKeys = (ownMatches || []).map(m => m.key);

  if (matchKeys.length) {
    const { error: msgErr } = await supabase.from('messages').delete().in('match_key', matchKeys);
    if (msgErr) throw msgErr;
  }
  const { error: matchErr } = await supabase
    .from('matches').delete().or(`user_id_1.eq.${user.id},user_id_2.eq.${user.id}`);
  if (matchErr) throw matchErr;

  const { error: likesErr } = await supabase
    .from('likes').delete().or(`from_user_id.eq.${user.id},to_user_id.eq.${user.id}`);
...
<style>
  ...
  body.dark-mode #auth-screen { --cream: #fbf3ef; --card: #ffffff; --ink: #2a1620; --muted: #8c7a83; --border: #f0e4e0; }
  body.dark-mode .swipe-card, body.dark-mode .empty-state { box-shadow: 0 10px 40px rgba(0,0,0,0.35); }
  * { box-sizing: border-box; }
  ...
  .q-card { width: 100%; max-width: 480px; background: var(--card); border-radius: 22px; padding: 32px; box-shadow: 0 25px 70px rgba(0,0,0,0.4); }
  ...
  .q-option {
    ...
    padding: 13px 16px; border: 1.5px solid var(--border); border-radius: 12px; background: var(--card);
  ...
  .search-box { flex: 1; max-width: 380px; display: flex; align-items: center; gap: 8px; background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 10px 14px; color: var(--muted); font-size: 13.5px; }
  .top-actions { display: flex; align-items: center; gap: 14px; }
  .icon-btn { width: 38px; height: 38px; border-radius: 50%; border: none; background: var(--card); display: flex; align-items: center; justify-content: center; position: relative; font-size: 16px; }
  ...
  .filter-tabs button { display: flex; align-items: center; gap: 6px; border: 1px solid var(--border); background: var(--card); padding: 9px 16px; border-radius: 20px; font-size: 13.5px; font-weight: 500; color: var(--muted); }
  .filter-tabs button.active { background: var(--maroon-800); color: white; border-color: var(--maroon-800); }

  .swipe-card { background: var(--card); border-radius: 22px; overflow: hidden; box-shadow: 0 10px 40px rgba(75,20,45,0.1); }
  ...
  .empty-state { background: var(--card); border-radius: 22px; padding: 60px 30px; text-align: center; color: var(--muted); }
  ...
  .act-pass { width: 62px; height: 62px; background: var(--card); color: #d8536b; font-size: 24px; }
  ...
  .side-card { background: var(--card); border-radius: 18px; padding: 18px; margin-bottom: 18px; }
  ...
  /* Chat view */
  .chat-panel { background: var(--card); border-radius: 22px; display: flex; flex-direction: column; height: 620px; }
  ...
  .admirer-card { background: var(--card); border-radius: 16px; padding: 16px; text-align: center; }
  ...
  /* Profile view */
  .profile-form { background: var(--card); border-radius: 18px; padding: 24px; max-width: 480px; }
  ...
  /* ---------- Settings ---------- */
  .settings-list { max-width: 480px; display: flex; flex-direction: column; gap: 18px; }
  .settings-section { background: var(--card); border-radius: 18px; padding: 6px 18px; }
  .settings-section-title { font-size: 12px; font-weight: 800; letter-spacing: 0.3px; color: var(--pink-600); margin: 14px 0 4px; }
  ...
  .settings-row-static { padding-bottom: 13px; }
  ...
  .settings-expand-btn {
    ...
    padding: 13px 0; font-size: 14px; font-weight: 600; color: var(--ink); display: flex; align-items: center; justify-content: space-between; gap: 10px;
  }
  .settings-expand-btn:last-child, .settings-section > .settings-expand-btn:last-of-type { border-bottom: none; }
  .settings-expand-btn .chevron { flex-shrink: 0; color: var(--muted); }
  .settings-card { padding: 4px 0 16px; }
  .settings-card label { display: block; font-size: 12px; font-weight: 600; color: var(--muted); margin-bottom: 4px; }
  .settings-card input { width: 100%; padding: 10px 12px; border: 1px solid var(--border); border-radius: 10px; margin-bottom: 12px; font-size: 13.5px; background: var(--card); color: var(--ink); }
  .settings-card > button { background: var(--pink-600); color: white; border: none; padding: 10px 18px; border-radius: 10px; font-weight: 600; font-size: 13px; }
  .settings-danger-zone { border: 1px solid rgba(214,56,104,0.25); }
  .settings-danger-copy { font-size: 12.5px; color: var(--muted); line-height: 1.5; margin: 4px 0 12px; }
  .settings-danger-btn { background: #d63868; color: white; border: none; padding: 10px 18px; border-radius: 10px; font-weight: 700; font-size: 13px; width: 100%; }
  ...
  @media (max-width: 720px) {
    ...
    .swipe-photo { height: 56vh; min-height: 320px; }
  ...
  .filter-input, .filter-select {
    ...
    font-size: 13.5px; font-family: inherit; color: var(--ink); background: var(--card); outline: none;
  }
  ...
  .confirm-modal {
    ...
    width: 100%; max-width: 420px; margin: 0 auto; background: var(--card); border-radius: 20px 20px 0 0;
    padding: 22px 20px calc(20px + env(safe-area-inset-bottom, 0px));
    box-shadow: 0 -12px 40px rgba(20,5,12,0.25);
    transform: translateY(100%); pointer-events: none; transition: transform 0.3s cubic-bezier(.32,.72,0,1);
  }
  .confirm-modal.open { transform: translateY(0); pointer-events: auto; }
  .confirm-modal h2 { margin: 0 0 8px; font-size: 18px; color: var(--ink); }
  .confirm-modal p { margin: 0 0 14px; font-size: 13.5px; color: var(--muted); line-height: 1.5; }
  .confirm-modal label { display: block; font-size: 12px; font-weight: 600; color: var(--muted); margin-bottom: 6px; }
  .confirm-modal input {
    width: 100%; padding: 11px 13px; border: 1.5px solid var(--border); border-radius: 10px;
    font-size: 14px; font-family: inherit; color: var(--ink); background: var(--card); margin-bottom: 16px;
  }
  .confirm-modal .confirm-actions { display: flex; gap: 10px; }
  .confirm-modal .confirm-cancel, .confirm-modal .confirm-danger {
    flex: 1; border: none; border-radius: 12px; padding: 13px; font-size: 14px; font-weight: 700;
    min-height: 46px; font-family: inherit;
  }
  .confirm-modal .confirm-cancel { background: var(--cream); color: var(--ink); }
  .confirm-modal .confirm-danger { background: #d63868; color: white; opacity: 0.45; transition: opacity 0.15s ease; }
  .confirm-modal .confirm-danger:not(:disabled) { opacity: 1; }

  @media (min-width: 721px) {
    .confirm-modal { left: 50%; bottom: 50%; transform: translate(-50%, 50%) translateY(20px); border-radius: 18px; max-width: 380px; opacity: 0; }
    .confirm-modal.open { transform: translate(-50%, 50%); opacity: 1; }
  }
  ...
  <main class="main">
    <div class="topbar">
      ...
      <div class="top-actions">
        ...
        <button class="icon-btn" id="settings-trigger-btn" onclick="switchView('settings')" aria-label="Settings" title="Settings" style="display:none;">
          <svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="3.2"/><path d="M19.4 13.5a1.7 1.7 0 0 0 .34 1.87l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.7 1.7 0 0 0-1.87-.34 1.7 1.7 0 0 0-1.04 1.56V19.5a2 2 0 1 1-4 0v-.09a1.7 1.7 0 0 0-1.04-1.56 1.7 1.7 0 0 0-1.87.34l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.7 1.7 0 0 0 .34-1.87 1.7 1.7 0 0 0-1.56-1.04H2.5a2 2 0 1 1 0-4h.09a1.7 1.7 0 0 0 1.56-1.04 1.7 1.7 0 0 0-.34-1.87l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.7 1.7 0 0 0 1.87.34H8.6a1.7 1.7 0 0 0 1.04-1.56V2.5a2 2 0 1 1 4 0v.09a1.7 1.7 0 0 0 1.04 1.56 1.7 1.7 0 0 0 1.87-.34l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.7 1.7 0 0 0-.34 1.87V8.6a1.7 1.7 0 0 0 1.56 1.04h.09a2 2 0 1 1 0 4h-.09a1.7 1.7 0 0 0-1.56 1.04Z"/></svg>
        </button>
    ...
    <div class="content-grid">
        ...
        <div class="view-panel" id="panel-settings">
          <div class="settings-list">
            ...
            <div class="settings-section">
              ...
              <div class="settings-row">
                <div class="settings-row-label">
                  ...
                  <span class="settings-row-desc">Switch the app to a dark color theme</span>
                </div>
                <button type="button" class="filter-switch" id="s-darkmode" onclick="toggleSetting('appearance','darkMode', this)"></button>
              </div>
            </div>
            ...
            <div class="settings-section">
              ...
              <div class="settings-row">
                <div class="settings-row-label">
                  <span class="settings-row-title">New matches</span>
                  <span class="settings-row-desc">Get notified when you match with someone</span>
                </div>
                <button type="button" class="filter-switch" id="s-notif-matches" onclick="toggleSetting('notifications','newMatches', this)"></button>
              </div>
              <div class="settings-row">
                <div class="settings-row-label">
                  <span class="settings-row-title">New messages</span>
                  <span class="settings-row-desc">Get notified about new chat messages</span>
                </div>
                <button type="button" class="filter-switch" id="s-notif-messages" onclick="toggleSetting('notifications','newMessages', this)"></button>
              </div>
              <div class="settings-row">
                <div class="settings-row-label">
                  <span class="settings-row-title">Likes</span>
                  <span class="settings-row-desc">Get notified when someone likes you</span>
                </div>
                <button type="button" class="filter-switch" id="s-notif-likes" onclick="toggleSetting('notifications','likes', this)"></button>
              </div>
              <div class="settings-row">
                <div class="settings-row-label">
                  <span class="settings-row-title">Email notifications</span>
                  <span class="settings-row-desc">Occasional email updates and digests</span>
                </div>
                <button type="button" class="filter-switch" id="s-notif-email" onclick="toggleSetting('notifications','email', this)"></button>
              </div>
            </div>
            ...
            <div class="settings-section">
              ...
              <div class="settings-row">
                <div class="settings-row-label">
                  <span class="settings-row-title">Show online status</span>
                  <span class="settings-row-desc">Let matches see when you're active</span>
                </div>
                <button type="button" class="filter-switch" id="s-privacy-online" onclick="toggleSetting('privacy','showOnlineStatus', this)"></button>
              </div>
              <div class="settings-row">
                <div class="settings-row-label">
                  <span class="settings-row-title">Show distance</span>
                  <span class="settings-row-desc">Show your approximate distance on your profile</span>
                </div>
                <button type="button" class="filter-switch" id="s-privacy-distance" onclick="toggleSetting('privacy','showDistance', this)"></button>
              </div>
            </div>
            ...
            <div class="settings-section">
              ...
              <div class="settings-row settings-row-static">
                <div class="settings-row-label">
                  <span class="settings-row-title">Email</span>
                  <span class="settings-row-desc" id="s-account-email">—</span>
                </div>
              </div>
              <button type="button" class="settings-expand-btn" onclick="toggleSettingsCard('password-card')">Change password <svg class="chevron" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M9 6l6 6-6 6"/></svg></button>
              <div class="settings-card" id="password-card" style="display:none;">
                <label>Current password</label>
                <input id="s-current-password" type="password" autocomplete="current-password" />
                <label>New password (min. 6 characters)</label>
                <input id="s-new-password" type="password" autocomplete="new-password" minlength="6" />
                <label>Confirm new password</label>
                <input id="s-confirm-password" type="password" autocomplete="new-password" minlength="6" />
                <button type="button" onclick="submitPasswordChange()">Update password</button>
                <span class="save-note" id="password-save-note" style="display:none;">Password updated!</span>
              </div>
              <button type="button" onclick="logout()" class="settings-expand-btn">Log out <svg class="chevron" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M9 6l6 6-6 6"/></svg></button>
            </div>

            <div class="settings-section settings-danger-zone">
              <p class="settings-section-title">Danger zone</p>
              <button type="button" class="settings-expand-btn" onclick="toggleSettingsCard('delete-card')">Delete account <svg class="chevron" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M9 6l6 6-6 6"/></svg></button>
              <div class="settings-card" id="delete-card" style="display:none;">
                <p class="settings-danger-copy">This permanently deletes your profile, matches, and messages. This cannot be undone.</p>
                <button type="button" class="settings-danger-btn" onclick="openDeleteAccountModal()">Permanently delete my account</button>
              </div>
            </div>
      ...
      <div class="confirm-overlay" id="confirm-overlay" onclick="closeDeleteAccountModal()"></div>
      <div class="confirm-modal" id="confirm-modal" role="dialog" aria-modal="true" aria-labelledby="confirm-modal-title">
        <h2 id="confirm-modal-title">Delete your account?</h2>
        <p>This permanently deletes your profile, matches, and messages. This cannot be undone.</p>
        <label for="confirm-delete-input">Type <b>DELETE</b> to confirm</label>
        <input id="confirm-delete-input" type="text" autocomplete="off" oninput="onDeleteConfirmInput()" />
        <div class="confirm-actions">
          <button type="button" class="confirm-cancel" onclick="closeDeleteAccountModal()">Cancel</button>
          <button type="button" class="confirm-danger" id="confirm-danger-btn" disabled onclick="submitDeleteAccount()">Delete my account</button>
        </div>
      </div>
...
function switchView(view) {
  ...
  const titles = { discover: 'Discover', matches: 'Messages', likes: 'Likes You', ai: 'Swing AI', profile: 'My Profile', settings: 'Settings' };
  document.getElementById('view-title').textContent = titles[view] || 'Discover';
  document.getElementById('filter-trigger-btn').style.display = view === 'discover' ? 'flex' : 'none';
  document.getElementById('settings-trigger-btn').style.display = view === 'profile' ? 'flex' : 'none';
  ...
  else if (view === 'settings') { document.getElementById('panel-settings').classList.add('visible'); renderSettings(); }
}
...
// Only fields that actually exist on the profile are used to exclude
// candidates; the rest are recorded in the filter state (and counted
// toward the badge) but do not affect this pass-through check yet.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  panel.addEventListener('click', (e) => {
    ...
    if (mode === 'multi') {
      ...
      state.filters[group] = (state.filters[group] === val) ? (group === 'showMe' ? 'everyone' : '') : val;
    }
    syncChipSelectedStates();
  });
  updateFilterBadge();
...
function applyDarkModeFromSettings() {
  const on = !!(state.user && state.user.settings && state.user.settings.appearance && state.user.settings.appearance.darkMode);
  document.body.classList.toggle('dark-mode', on);
}

function renderSettings() {
  const s = (state.user && state.user.settings) || {};
  const notif = s.notifications || {};
  const privacy = s.privacy || {};
  const appearance = s.appearance || {};

  document.getElementById('s-account-email').textContent = state.user.email || '—';

  setSwitch('s-darkmode', !!appearance.darkMode);
  setSwitch('s-notif-matches', notif.newMatches !== false);
  setSwitch('s-notif-messages', notif.newMessages !== false);
  setSwitch('s-notif-likes', notif.likes !== false);
  setSwitch('s-notif-email', notif.email !== false);
  setSwitch('s-privacy-online', privacy.showOnlineStatus !== false);
  setSwitch('s-privacy-distance', privacy.showDistance !== false);
}
...
async function toggleSetting(section, key, btnEl) {
  const next = !btnEl.classList.contains('on');
  btnEl.classList.toggle('on', next);

  // Optimistic local update so re-opening the panel reflects the change immediately.
  state.user.settings = state.user.settings || {};
  state.user.settings[section] = state.user.settings[section] || {};
  state.user.settings[section][key] = next;

  if (section === 'appearance' && key === 'darkMode') applyDarkModeFromSettings();

  try {
    const data = await api('/api/settings', { method: 'PUT', body: JSON.stringify({ [section]: { [key]: next } }) });
...
function toggleSettingsCard(id) {
  ...
  // Accordion behavior: opening one settings card closes any other open one.
  document.querySelectorAll('.settings-card').forEach(c => { if (c !== card) c.style.display = 'none'; });
...
async function submitPasswordChange() {
  ...
  try {
    ...
    const note = document.getElementById('password-save-note');
    note.style.display = 'inline';
...
function onDeleteConfirmInput() {
  const typed = document.getElementById('confirm-delete-input').value;
  document.getElementById('confirm-danger-btn').disabled = (typed !== 'DELETE');
}
