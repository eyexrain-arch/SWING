...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .discover-category-tabs { overflow-x: auto; overflow-y: hidden; flex-wrap: nowrap; -webkit-overflow-scrolling: touch; scrollbar-width: none; padding-bottom: 2px; margin-bottom: 12px; }
  .discover-category-tabs::-webkit-scrollbar { display: none; }
  /* Compact icon-only pills — one row, equal size, the active one in pink. */
  .discover-category-tabs button {
    flex: 0 0 auto; white-space: nowrap; width: 46px; height: 46px; padding: 0;
  ...
  .discover-stack.discover-stack--carousel { min-height: 525px; }
  ...
  /* Fixed (not viewport-scaled) card width — every card is the exact same
     size, sized large so one card fills most of the view and the next
     card's edge peeks in as a scroll cue. */
  .discover-carousel-card {
    flex: 0 0 420px; width: 420px; scroll-snap-align: start; scroll-snap-stop: always;
    display: flex; flex-direction: column; align-items: center; gap: 12px;
  }
  @media (max-width: 900px) {
    .discover-carousel-card { flex-basis: 360px; width: 360px; }
    .discover-stack.discover-stack--carousel { min-height: 450px; }
  }
  @media (max-width: 560px) {
    .discover-carousel-card { flex-basis: 300px; width: 300px; }
    .discover-stack.discover-stack--carousel { min-height: 375px; }
  }
  @media (max-width: 380px) {
    .discover-carousel-card { flex-basis: 260px; width: 260px; }
    .discover-stack.discover-stack--carousel { min-height: 325px; }
  }
...
function renderDiscoverList() {
  ...
  const cards = list.map(c => \`
      <div class="discover-carousel-card" data-id="\${c.id}">
        <div onclick="openCandidateDetail('\${c.id}')">\${buildCandidateMiniPhotoHTML(c)}</div>
      </div>\`).join('');
