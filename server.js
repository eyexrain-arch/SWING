...
function renderDiscoverList() {
  ...
  list.innerHTML = state.candidates.map(c => {
    const photoStyle = c.photoUrl
      ...
      : \`background:linear-gradient(135deg, \${colorFor(c.name)}, #2a1620);\`;
    const tags = (c.tags || []).slice(0, 4).map(t => '<span>' + t + '</span>').join('');
    return \`
      <div class="discover-row" onclick="openCandidateDetail('\${c.id}')">
        ...
        <div class="discover-row-body">
          <div class="discover-row-name">\${c.name}\${c.age ? ', ' + c.age : ''}</div>
          \${c.location ? '<div class="discover-row-loc"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M12 21s7-6.5 7-11.5A7 7 0 1 0 5 9.5C5 14.5 12 21 12 21Z"/><circle cx="12" cy="9.5" r="2.2"/></svg> ' + c.location + '</div>' : ''}
          \${c.bio ? '<div class="discover-row-bio">' + c.bio + '</div>' : ''}
          \${tags ? '<div class="discover-row-tags">' + tags + '</div>' : ''}
        </div>
        <button class="discover-row-like" onclick="event.stopPropagation(); discoverListAction('\${c.id}','like')" aria-label="Like \${c.name}" title="Like">
