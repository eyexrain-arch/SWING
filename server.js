...
function logout() {
  stopChatPolling();
  if (state.backgroundPollTimer) clearInterval(state.backgroundPollTimer);
  localStorage.removeItem('swing_token');
  state = { token: null, user: null, candidates: [], matches: [], admirers: [], activeChat: null, chatPollTimer: null, backgroundPollTimer: null };
...
async function enterDashboard() {
  ...
  state.backgroundPollTimer = setInterval(() => {
    if (!state.activeChat) loadMatches();
  }, 10000);
}
...
async function openChat(matchKey) {
  ...
  state.chatPollTimer = setInterval(() => {
    if (state.activeChat === matchKey) loadChatMessages(matchKey);
  }, 3000);
}
