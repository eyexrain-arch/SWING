...
async function handleRegister(req, res) {
  ...
  if (typeof password !== 'string' || password.length < 10 || password.length > 128) {
    return sendJSON(res, 400, { error: 'Password must be 10–128 characters' });
  }
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .compat-reveal-btn {
    width: 100%; padding: 11px; margin-top: 2px; border-radius: 10px; border: 1px dashed var(--border);
    background: var(--cream); color: var(--ink); font-weight: 600; font-size: 13px; cursor: pointer;
  }
  .compat-reveal-btn:hover { background: var(--border); }
    ...
    <div class="swipe-card">
      ...
      <div class="swipe-card-body">
        ...
        <div class="swipe-section">
          ...
          <button type="button" class="compat-reveal-btn" onclick="event.stopPropagation(); toggleCompatAnswers(this)">Want to know more? Tap to see their answers</button>
          <div class="compat-answers-body" style="display:none;">
...
function attachCardDragHandlers(el) {
  ...
  function onPointerUp(e) {
    ...
    // Release capture immediately — otherwise the browser may also redirect
    // the resulting "click" event to this card instead of whatever was
    // actually tapped inside it.
    ...
    if (Math.abs(dx) > SWIPE_THRESHOLD) {
      ...
      setStamps(0);
      if (isTap) {
        // Belt-and-suspenders: even after releasing capture, some browsers
        // still don't reliably re-target the follow-up click to the actual
        // tapped element while a pointer was captured. So find whatever is
        // visually under the release point ourselves and invoke it directly
        // (e.g. a vibes photo thumbnail) instead of depending on click
        // retargeting behavior that varies by browser.
        const target = document.elementFromPoint(e.clientX, e.clientY);
        const tapTarget = target && target.closest && target.closest('.swipe-vibes-thumb, .compat-reveal-btn');
...
function toggleCompatAnswers(btn) {
  const body = btn.nextElementSibling;
  if (!body) return;
  const hidden = body.style.display === 'none';
  body.style.display = hidden ? '' : 'none';
  btn.textContent = hidden ? 'Hide their answers' : 'Want to know more? Tap to see their answers';
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (apiRequest) {
    const generalLimit = pathname === '/api/login' || pathname === '/api/register' ? 10 : 120;
