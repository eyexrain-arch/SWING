...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .about-quiz-block { margin-top: 16px; }
  .about-quiz-heading { font-size: 12px; font-weight: 700; color: var(--muted); text-transform: uppercase; letter-spacing: .04em; margin: 0 0 9px; }
  .about-quiz-card { margin-bottom: 8px; }
  .about-quiz-card:last-child { margin-bottom: 0; }
  .about-pill {
        ...
        <div class="view-panel" id="panel-profile">
          ...
          <div class="profile-view" id="profile-view-mode">
            ...
            <div class="about-card">
              ...
              <div class="about-quiz-block" id="ph-quiz-answers"></div>
            </div>
    ...
    <div class="swipe-card">
      ...
      <div class="swipe-card-body">
        <div class="swipe-section">
          ...
          <div class="about-quiz-block">
            <div class="about-quiz-heading">Compatibility questions</div>
            \${c.quizAnswers.map(q => '<div class="prompt-view-card about-quiz-card"><div class="prompt-view-question">' + q.question + '</div><div class="prompt-view-answer">' + q.answer + '</div></div>').join('')}
          </div>\` : ''}
        </div>
...
function toggleMultiAnswer(qId, value) {
  ...
  // "No dealbreakers" is exclusive with the specific dealbreakers.
  if (qId === 'dealbreakers') {
    if (value === 'none') {
      state.questionnaireAnswers[qId] = idx === -1 ? ['none'] : [];
      renderQuestionnaireStep();
      return;
    }
  ...
  if (idx === -1) arr.push(value); else arr.splice(arr.indexOf(value), 1);
  state.questionnaireAnswers[qId] = arr;
  renderQuestionnaireStep();
}

function hasQuestionnaireAnswer(q) {
  const value = state.questionnaireAnswers[q.id];
...
async function goNextQuestion() {
  const q = state.questionnaireQuestions[state.questionnaireStep];
  if (!hasQuestionnaireAnswer(q)) {
...
async function finishQuestionnaire() {
  const unanswered = state.questionnaireQuestions.find(q => !hasQuestionnaireAnswer(q));
  if (unanswered) {
    alert('Please answer every compatibility question before finishing.');
    state.questionnaireStep = state.questionnaireQuestions.indexOf(unanswered);
    renderQuestionnaireStep();
    return;
  }
  ...
  try {
    ...
    alert(e.message || 'Could not save your questionnaire answers. Please try again.');
    return;
  }
  await closeQuestionnaireScreen();
}
...
// Show every completed compatibility-question answer inside About Me.
function renderQuizAnswersInAbout(u) {
  const el = document.getElementById('ph-quiz-answers');
  if (!el) return;
  const answers = (u && u.quizAnswers) || [];
  if (!answers.length) {
    el.innerHTML = '';
    return;
  }
  el.innerHTML = '<div class="about-quiz-heading">Compatibility questions</div>' +
    answers.map(q =>
      '<div class="prompt-view-card about-quiz-card">' +
        '<div class="prompt-view-question">' + q.question + '</div>' +
        '<div class="prompt-view-answer">' + q.answer + '</div>' +
      '</div>'
    ).join('');
}
