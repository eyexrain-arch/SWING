...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .q-header .q-kicker { font-weight: 700; color: var(--pink-600); font-size: 12px; letter-spacing: 1px; margin-top: 4px; }
  ...
  .compat-panel .compat-percent { font-weight: 700; color: var(--pink-600); font-size: 15px; margin-bottom: 6px; }
...
<div id="questionnaire-screen">
  <div class="q-card">
    <div class="q-header">
      ...
      <div class="q-kicker">✨ COMPATIBILITY QUESTIONNAIRE</div>
    </div>
...
<div id="dashboard">
  <aside class="sidebar">
    ...
    <button class="nav-item" data-view="ai" onclick="switchView('ai')"><span class="icon">✨</span> Swing AI</button>
    ...
    <div class="content-grid">
      <div class="discover-col">
        ...
        <div class="view-panel" id="panel-ai">
          <div class="chat-panel">
            <div class="chat-header">
              <button class="back-btn" onclick="switchView('discover')">←</button>
              <div class="msg-avatar" style="width:36px;height:36px;font-size:16px;background:linear-gradient(135deg,var(--pink-500),var(--violet));">✨</div>
              <div style="font-weight:600;">Swing AI</div>
            </div>
            <div class="chat-messages" id="ai-chat-messages"></div>
            <div class="chat-input-row">
              <button id="ai-mic-btn" onclick="toggleVoiceInput()" style="background:var(--cream);color:var(--pink-600);border:none;width:42px;border-radius:10px;font-size:16px;flex-shrink:0;">🎙️</button>
        ...
        <div class="view-panel" id="panel-profile">
          <div class="profile-form">
            ...
            <div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border);">
              <button type="button" onclick="showQuestionnaire('profile')" style="background:var(--cream);color:var(--ink);border:1px solid var(--border);width:100%;">✨ Compatibility Quiz</button>
            </div>
          </div>
        </div>
  ...
  <nav class="mobile-nav">
    ...
    <button class="mobile-nav-item mobile-nav-fab" data-view="ai" onclick="switchView('ai')"><span class="icon">✨</span><span class="fab-label">AI</span></button>
  ...
  card.innerHTML = \`
    <div class="swipe-card">
      <div class="swipe-photo" style="\${photoStyle}">
        ...
        \${c.compatibility !== null && c.compatibility !== undefined ? \`<div class="photo-counter compat-badge" style="position:absolute;top:16px;right:16px;">💫 \${c.compatibility}% Match</div>\` : ''}
  ...
  if (match && match.compatibility !== null && match.compatibility !== undefined) {
    compatEl.innerHTML = \`
      <div class="compat-panel">
        <div class="compat-percent">💫 \${match.compatibility}% Compatible</div>
...
function renderAiChat() {
  ...
  if (!state.aiChatHistory.length) {
    el.innerHTML = '<div class="muted-note" style="text-align:center;margin-top:20px;">Tell me what you are looking for in a partner, and I will help you find good matches from your Discover list. 💫</div>';
  } else {
...
async function sendAiMessage(overrideText) {
  ...
  try {
    ...
    const typingEl = document.getElementById(typingId);
    if (typingEl) typingEl.remove();
    state.aiChatHistory.push({ role: 'assistant', content: '⚠️ ' + err.message });
    renderAiChat();
  }
}
  ...
  if (q.type === 'single') {
    optionsHtml = q.options.map(o => \`
      <button type="button" class="q-option \${selected === o.value ? 'selected' : ''}" onclick="selectSingleAnswer('\${q.id}','\${o.value}')">
        <span>\${o.label}</span>\${selected === o.value ? '<span class="check">✓</span>' : ''}
      </button>
    \`).join('');
  } else {
    ...
    optionsHtml = q.options.map(o => \`
      <button type="button" class="q-option \${arr.includes(o.value) ? 'selected' : ''}" onclick="toggleMultiAnswer('\${q.id}','\${o.value}')">
        <span>\${o.label}</span>\${arr.includes(o.value) ? '<span class="check">✓</span>' : ''}
      </button>
    \`).join('');
  }
