...
function toPublicUser(row) {
  ...
  return {
    ...
    // Optional prompt/question fields. They are only awarded points when the
    // corresponding data actually exists in the user's row.
    prompts: row.prompts || [],
    profilePrompts: row.profile_prompts || [],
...
const PROFILE_STRENGTH_SECTIONS = [
  ...
  },
  {
    ...
    weight: 10,
    // Supports prompt data if/when the profile schema contains it. This keeps
    // the calculation data-driven without awarding points for a missing field.
    fields: [
      { get: u => u.prompts },
      { get: u => u.profilePrompts },
      { get: u => u.promptAnswers },
      { get: u => u.questions },
    ],
  },
];
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .profile-completion { margin-bottom: 18px; }
  .profile-completion-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 7px; }
  .profile-completion-row span:first-child { font-size: 12.5px; font-weight: 700; color: var(--muted); }
  .profile-completion-row span:last-child { font-size: 12.5px; font-weight: 800; color: var(--pink-600); }
  #profile-completion-cta {
    display: none; background: var(--pink-100); color: var(--pink-600); border: 1.5px solid var(--pink-500);
    width: 100%; padding: 10px; border-radius: 9px; font-weight: 700; font-size: 12.5px; margin-top: 10px; cursor: pointer;
  }
  ...
  /* iOS / Android-style Settings control: compact, soft, tactile, and clean */
  .icon-btn#profile-settings-btn {
    ...
    border: 1px solid rgba(30,30,35,.08);
  ...
  .icon-btn#profile-settings-btn svg {
    width: 20px;
    height: 20px;
    stroke-width: 1.9;
  ...
  .icon-btn#profile-settings-btn:hover {
    background: #f7f7f8;
    color: var(--pink-600);
    box-shadow: 0 4px 12px rgba(0,0,0,.09), 0 1px 3px rgba(0,0,0,.04);
  ...
  .icon-btn#profile-settings-btn:focus-visible {
    outline: 3px solid rgba(179,36,90,.16);
    outline-offset: 2px;
  }
  ...
  .swipe-hint { text-align: center; font-size: 12.5px; color: var(--muted); min-height: 18px; }
  .discover-empty { background: white; border: 1px solid var(--border); border-radius: 24px; padding: 56px 28px; text-align: center; box-shadow: 0 10px 34px rgba(75,20,45,.07); }
  .discover-empty-icon { width: 56px; height: 56px; margin: 0 auto 16px; display:flex; align-items:center; justify-content:center; border-radius:18px; background: var(--pink-100); color: var(--pink-600); font-size:24px; }
  .discover-empty h3 { margin:0 0 7px; font-size:20px; color:var(--ink); }
  .discover-empty p { margin:0 auto 18px; max-width:380px; color:var(--muted); font-size:13.5px; line-height:1.55; }
  .discover-empty-btn { border:0; border-radius:12px; padding:11px 17px; background:var(--pink-600); color:white; font-weight:700; cursor:pointer; }
  .swipe-actions button { cursor:pointer; transition:transform .16s ease, box-shadow .16s ease; -webkit-tap-highlight-color:transparent; }
  .swipe-actions button:hover { transform:translateY(-2px); box-shadow:0 9px 22px rgba(0,0,0,.12); }
  .swipe-actions button:active { transform:scale(.94); }
  .swipe-actions button:focus-visible { outline:3px solid rgba(179,36,90,.18); outline-offset:3px; }
  ...
  .profile-header-card .profile-completion { text-align: left; margin: 20px 0 4px; }
  .profile-edit-btn {
  ...
  body.settings-dark .discover-empty { background: var(--card); border-color: var(--border); }
  body.settings-dark .discover-empty h3 { color: var(--ink); }
    ...
    <div class="topbar">
      ...
      <div class="top-actions">
        ...
        <button class="icon-btn" id="profile-settings-btn" onclick="openProfileSettings()" aria-label="Settings" title="Settings" style="display:none;">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="3.2"/><path d="M19.4 13.5a1.7 1.7 0 0 0 .34 1.87l.06.06a2.06 2.06 0 1 1-2.92 2.92l-.06-.06a1.7 1.7 0 0 0-1.87-.34 1.7 1.7 0 0 0-1.03 1.56V19.6a2.06 2.06 0 1 1-4.12 0v-.09a1.7 1.7 0 0 0-1.11-1.56 1.7 1.7 0 0 0-1.87.34l-.06.06a2.06 2.06 0 1 1-2.92-2.92l.06-.06a1.7 1.7 0 0 0 .34-1.87 1.7 1.7 0 0 0-1.56-1.03H4.4a2.06 2.06 0 1 1 0-4.12h.09A1.7 1.7 0 0 0 6.05 6.98a1.7 1.7 0 0 0-.34-1.87l-.06-.06a2.06 2.06 0 1 1 2.92-2.92l.06.06a1.7 1.7 0 0 0 1.87.34H10.6a1.7 1.7 0 0 0 1.03-1.56V4.4a2.06 2.06 0 1 1 4.12 0v.09a1.7 1.7 0 0 0 1.03 1.56 1.7 1.7 0 0 0 1.87-.34l.06-.06a2.06 2.06 0 1 1 2.92 2.92l-.06.06a1.7 1.7 0 0 0-.34 1.87V10.6a1.7 1.7 0 0 0 1.56 1.03h.09a2.06 2.06 0 1 1 0 4.12h-.09a1.7 1.7 0 0 0-1.56 1.03Z"/></svg>
        </button>
        ...
        <div class="view-panel" id="panel-profile">
          ...
          <div class="profile-view" id="profile-view-mode">
            <div class="profile-header-card">
              ...
              <div class="profile-completion" id="profile-completion-block">
                <div class="profile-completion-row">
                  <span>Profile strength</span>
                  <span id="profile-completion-pct">0%</span>
                </div>
                <div class="q-progress-bar"><div class="q-progress-fill" id="profile-completion-fill" style="width:0%;"></div></div>
                <button type="button" id="profile-completion-cta" onclick="showDetailsScreen('profile')">Complete your profile →</button>
              </div>
            ...
            <div class="about-card">
              <div class="about-card-header">
                <span class="icon-circle"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><rect x="3" y="4" width="18" height="16" rx="2.5"/><path d="M3 9.5h18M8 4v3M16 4v3"/></svg></span>
                <h3>Quick Info</h3>
              </div>
              <div class="quick-info-grid" id="ph-quickinfo-grid"></div>
            </div>
...
function matchesFilters(c, f) {
  ...
  if (typeof c.age === 'number' && (c.age < f.ageMin || c.age > f.ageMax)) return false;
  if (f.country && f.country.trim() && !(c.country || '').toLowerCase().includes(f.country.trim().toLowerCase())) return false;
  if (f.city && f.city.trim() && !(c.location || '').toLowerCase().includes(f.city.trim().toLowerCase())) return false;

  // For profile fields, an active preference should be a real filter: an
  // unanswered profile does not qualify for a specific preference.
  if (f.interests && f.interests.length) {
    const tags = (c.tags || []).map(t => String(t).toLowerCase());
    if (!f.interests.some(i => tags.includes(String(i).toLowerCase()))) return false;
  }
  if (f.languages && f.languages.length) {
    const langs = (c.languages || []).map(l => String(l).toLowerCase());
    if (!f.languages.some(l => langs.includes(String(l).toLowerCase()))) return false;
  }
  if (f.education && f.education !== c.education) return false;
  if (f.exercise && f.exercise !== c.exercise) return false;
  if (f.drinking && f.drinking !== c.drinking) return false;
  if (f.smoking && f.smoking !== c.smoking) return false;
  if (f.religion && f.religion !== c.religion) return false;
  if (f.political && f.political !== c.political) return false;
  if (f.children && f.children !== c.children) return false;
  if (f.wantsChildren && f.wantsChildren !== c.wantsChildren) return false;
  if (typeof c.heightCm === 'number' && (c.heightCm < f.heightMin || c.heightCm > f.heightMax)) return false;
  if ((f.heightMin !== FILTER_DEFAULTS.heightMin || f.heightMax !== FILTER_DEFAULTS.heightMax) && typeof c.heightCm !== 'number') return false;
  if (f.datingIntentions && f.datingIntentions.length) {
    const theirs = Array.isArray(c.datingIntentions) ? c.datingIntentions : [];
    if (!f.datingIntentions.some(i => theirs.includes(i))) return false;
  }
  if (f.newHere) {
  ...
  // distance, verified and onlineNow require location/admin/presence data that
  // the current schema does not provide. Do not silently fake those filters.
  return true;
}
...
function renderSwipeCard() {
  ...
  if (!count) {
    const hasFilters = countActiveFilters(state.filters) > 0;
    card.innerHTML = \`
      <div class="discover-empty">
        <div class="discover-empty-icon">♥</div>
        <h3>${hasFilters ? 'No one matches your filters' : 'You’re all caught up'}</h3>
        <p>${hasFilters ? 'Try widening your preferences to see more people.' : 'There are no new profiles to show right now. Check back soon.'}</p>
        ${hasFilters ? '<button type="button" class="discover-empty-btn" onclick="resetFilters();closeFilterPanel();">Reset filters</button>' : ''}
      </div>\`;
    hint.textContent = '';
    return;
  }
  ...
  const safeBio = escapeHtml(c.bio || '');
  const safeLocation = escapeHtml(c.location || '');
  const safeTag = escapeHtml(c.tags && c.tags.length ? c.tags[0] : 'New here');
  const photoUrl = safeHttpUrl(c.photoUrl);
  ...
  const gallery = Array.isArray(c.galleryUrls) ? c.galleryUrls.slice(0, 3).map(safeHttpUrl).filter(Boolean) : [];
  const compat = Number.isFinite(Number(c.compatibility)) ? Math.round(Number(c.compatibility)) : null;

  hint.textContent = \`${count} profile${count === 1 ? '' : 's'} available\`;
  card.innerHTML = \`
    <div class="swipe-card">
      <div class="swipe-photo" style="${photoStyle}">
        <div class="photo-tag"><svg viewBox="0 0 24 24" width="11" height="11" fill="currentColor" stroke="none" aria-hidden="true"><path d="M12 2c.6 3.6 1.4 5.9 2.6 7.4 1.5 1.2 3.8 2 7.4 2.6-3.6.6-5.9 1.4-7.4 2.6-1.2 1.5-2 3.8-2.6 7.4-.6-3.6-1.4-5.9-2.6-7.4C7.9 13.4 5.6 12.6 2 12c3.6-.6 5.9-1.4 7.4-2.6C10.6 7.9 11.4 5.6 12 2Z"/></svg> ${safeTag}</div>
        ${compat !== null ? '<div class="photo-counter compat-badge">✦ ' + compat + '% Match</div>' : ''}
        <div class="swipe-details">
          <div class="name-row">${safeName}${c.age ? ', ' + escapeHtml(c.age) : ''}</div>
          ${safeLocation ? '<div class="meta"><span>⌖ ' + safeLocation + '</span></div>' : ''}
        </div>
      </div>
      <div class="swipe-card-body">
        <div class="swipe-section">
          <div class="swipe-section-title"><span class="icon-circle">♙</span>About Me</div>
          <p class="swipe-section-text ${safeBio ? '' : 'empty'}">${safeBio || 'No bio yet.'}</p>
        </div>
        ${tags.length ? '<div class="swipe-section"><div class="swipe-section-title"><span class="icon-circle">✦</span>Interests</div><div class="interests-chips">' + tags.map(t => '<span class="interest-chip">' + escapeHtml(t) + '</span>').join('') + '</div></div>' : ''}
        ${gallery.length ? '<div class="swipe-section"><div class="swipe-section-title"><span class="icon-circle">▣</span>My Vibes</div><div class="swipe-vibes-grid">' + gallery.map((url, i) => '<div class="swipe-vibes-thumb"><img src="' + escapeHtml(url) + '" alt="Vibes photo ' + (i + 1) + '" loading="lazy" /></div>').join('') + '</div></div>' : ''}
      </div>
    </div>\`;
}
...
const PROFILE_STRENGTH_SECTIONS = [
  ...
  { key: 'prompts', weight: 10, fields: [
    u => u.prompts, u => u.profilePrompts, u => u.promptAnswers, u => u.questions,
  ] },
];
...
function renderQuickInfo(u) {
  const el = document.getElementById('ph-quickinfo-grid');
  if (!el) return;
  const items = [];
  if (u.gender) items.push(['gender', 'Gender', titleCaseWords(u.gender)]);
  if (u.languages && u.languages.length) items.push(['languages', 'Languages', u.languages.join(', ')]);
  if (u.education) items.push(['education', 'Education', titleCaseWords(u.education)]);
  if (u.heightCm) items.push(['heightCm', 'Height', u.heightCm + ' cm']);
  if (u.exercise) items.push(['exercise', 'Exercise', titleCaseWords(u.exercise)]);
  if (u.drinking) items.push(['drinking', 'Drinking', titleCaseWords(u.drinking)]);
  if (u.smoking) items.push(['smoking', 'Smoking', titleCaseWords(u.smoking)]);
  if (u.religion) items.push(['religion', 'Religion', titleCaseWords(u.religion)]);
  if (u.political) items.push(['political', 'Politics', titleCaseWords(u.political)]);
  if (u.children) items.push(['children', 'Children', titleCaseWords(u.children)]);
  if (u.wantsChildren) items.push(['wantsChildren', 'Wants kids', titleCaseWords(u.wantsChildren)]);
  if (u.datingIntentions && u.datingIntentions.length) items.push(['datingIntentions', 'Looking for', u.datingIntentions.map(titleCaseWords).join(', ')]);
  if (!items.length) {
    el.innerHTML = '<p class="quick-info-card-text empty">Complete your profile to fill this in.</p>';
    return;
  }
  el.innerHTML = items.map(([key, label, value]) => \`
    <div class="quick-info-item">
      <span class="icon-circle">\${QUICK_INFO_ICONS[key] || ''}</span>
      <span class="qi-text"><span class="qi-label">\${label}</span><span class="qi-value">\${value}</span></span>
    </div>
  \`).join('');
}
