...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .discover-carousel-photo {
    ...
    box-shadow: 0 10px 40px rgba(75,20,45,0.1); cursor: pointer;
  }
  ...
  .carousel-like-btn {
    ...
    display: flex; align-items: center; justify-content: center;
    box-shadow: 0 6px 20px rgba(75,20,45,0.35); cursor: pointer;
  }
  .carousel-like-btn.liked-pop { animation: carousel-like-pop 0.35s ease; }
  @keyframes carousel-like-pop { 0% { transform: scale(1); } 50% { transform: scale(1.3); } 100% { transform: scale(1); } }
  ...
  @media (prefers-reduced-motion: reduce) {
    ...
    .carousel-like-btn.liked-pop { animation: none; }
  }
      ...
      <div class="discover-col">
        ...
        <div class="view-panel visible" id="panel-discover">
          ...
          <div class="discover-stack-wrap">
            ...
            <div class="swipe-hint" id="discover-stack-hint" style="display:none;">Scroll sideways to browse — tap ♥ on a profile to like it</div>
          </div>
        </div>
...
function createFreshState() {
  return {
    ...
    swipeBusy: false, // guards against double-tapping a carousel card's like button
  };
}
...
function renderDiscoverList() {
  ...
  const showCarouselUi = tab === 'maylike' && state.candidates.length > 0;
  ...
  // photo cards (picture, name, age, location). There's no pass/reject
  // step — scrolling past a card simply moves on to the next one; tapping
  // the heart below a card likes it, and tapping the card itself opens
  // the full profile.
  const heartIcon = '<svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor" stroke="none" aria-hidden="true" focusable="false"><path d="M12 20.5s-7.5-4.6-10-9.3C.4 8 1.7 4.6 5 3.6c2.2-.6 4.3.3 5.6 2.1l1.4 1.9 1.4-1.9C14.7 3.9 16.8 3 19 3.6c3.3 1 4.6 4.4 3 7.6-2.5 4.7-10 9.3-10 9.3Z"/></svg>';
  const cards = state.candidates.map(c => \`
      <div class="discover-carousel-card" data-id="\${c.id}">
        ...
        <button type="button" class="carousel-like-btn" onclick="carouselLikeCandidate('\${c.id}')" aria-label="Like \${escapeHtml(c.name)}" title="Like">\${heartIcon}</button>
      </div>\`).join('');
...
// browsing aid for pointer/keyboard users — it never records a like.
...
// Heart button on a "may like" carousel card. Plays a quick pop on the
// button, then commits the like and drops the card out of the filmstrip.
async function carouselLikeCandidate(id) {
  ...
  const btn = document.querySelector(\`.discover-carousel-card[data-id="\${id}"] .carousel-like-btn\`);
  if (btn) {
    btn.disabled = true;
  ...
  try {
    ...
    if (btn) { btn.disabled = false; btn.classList.remove('liked-pop'); }
    alert(err.message);
  } finally {
