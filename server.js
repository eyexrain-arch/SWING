      ...
      <div class="discover-col">
        ...
        <div class="view-panel" id="panel-profile">
          ...
          <div class="profile-form" id="profile-edit-mode" style="display:none;">
            ...
            <div class="interests-chips" id="p-interests-preview" style="margin:2px 0 12px;"></div>
            <label>Bio</label><textarea id="p-bio"></textarea>
            <div class="field-hint" style="margin:-8px 0 14px 2px;">Showing your first 4 interests. To change them, use the Interests card on your profile — tap "Edit" there.</div>
...
function fillProfileForm() {
  ...
  renderProfileFormInterestsPreview(state.user);
  refreshProfileViewMode();
  updateProfileCompletionDisplay();
}

// Read-only preview of the interests the user picked (shown under Location
// in the edit-profile form). Interests themselves are still edited from the
// separate Interests card/panel — this just surfaces the first 4 here so
// people can see them without leaving the edit form.
function renderProfileFormInterestsPreview(u) {
  const el = document.getElementById('p-interests-preview');
  if (!el) return;
  const tags = ((u && u.tags) || []).slice(0, 4);
  el.innerHTML = tags.length
    ? tags.map(t => \`<span class="interest-chip">\${escapeHtml(t)}</span>\`).join('')
    : '<p class="interests-empty-note">No interests picked yet.</p>';
}
