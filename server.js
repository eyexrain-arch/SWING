...
async function handleDiscover(req, res, query) {
  ...
  const candidates = (allUsers || [])
    ...
    .map(u => {
      ...
      const profileStrength = computeProfileStrength({ ...pub, questionnaire: u.questionnaire || {} });
      return { ...pub, profileCompletion: profileStrength.percentage, compatibility: compat ? compat.percent : null };
    });
...
async function handleMatches(req, res) {
  ...
  for (const m of matchRows || []) {
    ...
    const otherPublic = other ? toPublicUser(other) : null;
    const otherProfileStrength = otherPublic
      ? computeProfileStrength({ ...otherPublic, questionnaire: other.questionnaire || {} })
      : null;

    results.push({
      matchKey: m.key,
      createdAt: m.created_at,
      user: otherPublic ? { ...otherPublic, profileCompletion: otherProfileStrength.percentage } : null,
          ...
          <div class="profile-view" id="profile-view-mode">
            <div class="profile-header-card">
              <div class="profile-header-topbar">
                <button type="button" class="profile-settings-icon-btn" onclick="openProfileSettings()" aria-label="Settings" title="Settings">
                  <svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="3.2"/><path d="M19.4 13.5a1.7 1.7 0 0 0 .34 1.87l.06.06a2.06 2.06 0 1 1-2.92 2.92l-.06-.06a1.7 1.7 0 0 0-1.87-.34 1.7 1.7 0 0 0-1.03 1.56V19.6a2.06 2.06 0 1 1-4.12 0v-.09a1.7 1.7 0 0 0-1.11-1.56 1.7 1.7 0 0 0-1.87.34l-.06.06a2.06 2.06 0 1 1-2.92-2.92l.06-.06a1.7 1.7 0 0 0 .34-1.87 1.7 1.7 0 0 0-1.56-1.03H4.4a2.06 2.06 0 1 1 0-4.12h.09A1.7 1.7 0 0 0 6.05 6.98a1.7 1.7 0 0 0-.34-1.87l-.06-.06a2.06 2.06 0 1 1 2.92-2.92l.06.06a1.7 1.7 0 0 0 1.87.34H10.6a1.7 1.7 0 0 0 1.03-1.56V4.4a2.06 2.06 0 1 1 4.12 0v.09a1.7 1.7 0 0 0 1.03 1.56 1.7 1.7 0 0 0 1.87-.34l.06-.06a2.06 2.06 0 1 1 2.92 2.92l-.06.06a1.7 1.7 0 0 0-.34 1.87V10.6a1.7 1.7 0 0 0 1.56 1.03h.09a2.06 2.06 0 1 1 0 4.12h-.09a1.7 1.7 0 0 0-1.56 1.03Z"/></svg>
                </button>
              </div>
  ...
  card.innerHTML = \`
    <div class="swipe-card">
      <div class="swipe-photo" style="\${photoStyle}">
        ...
        \${typeof c.profileCompletion === 'number' ? \`<div class="photo-counter profile-strength-badge" style="position:absolute;top:16px;right:16px;transform:translateY(42px);">✦ \${c.profileCompletion}% Strength</div>\` : ''}
...
function defaultDetails() {
  return {
    languages: [], education: '', heightCm: null, exercise: '',
    drinking: '', smoking: '', religion: '', political: '',
...
function populateDetailsFormFromUser(user) {
  ...
  document.getElementById('d-height').value = state.details.heightCm ?? 170;
  document.getElementById('d-height-val').textContent = (state.details.heightCm ?? 170) + ' cm';
...
function openProfileSettings() {
  // Placeholder — wire this up to a real settings panel (account, privacy,
  // notifications, logout, etc.) whenever that screen gets built.
