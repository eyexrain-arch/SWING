...
function discoverOverlapScore(a, b) {
  const aa = Array.isArray(a) ? a.map(x => String(x).toLowerCase().trim()).filter(Boolean) : [];
  const bb = Array.isArray(b) ? b.map(x => String(x).toLowerCase().trim()).filter(Boolean) : [];
  if (!aa.length || !bb.length) return 0;
  const setB = new Set(bb);
  return Math.round((aa.filter(x => setB.has(x)).length / Math.max(aa.length, bb.length)) * 100);
}
function discoverFreshnessScore(createdAt) {
  if (!createdAt) return 50;
  const days = Math.max(0, (Date.now() - new Date(createdAt).getTime()) / 86400000);
  return Math.max(0, Math.round(100 * Math.exp(-days / 30)));
}
async function handleDiscover(req, res, query) {
  const user = await requireAuth(req, res);
  if (!user) return;
  const { data: swiped, error: swipedErr } = await supabase.from('likes').select('to_user_id').eq('from_user_id', user.id);
  ...
  const candidates = (allUsers || [])
    ...
    .map(u => {
      ...
      const interestScore = discoverOverlapScore(user.tags, u.tags);
      const intentionScore = discoverOverlapScore(user.dating_intentions, u.dating_intentions);
      const profileStrength = computeProfileStrength(publicUser).percentage;
      const freshness = discoverFreshnessScore(u.created_at);
      const discoverScore = Math.round((compat ? compat.percent : 0) * 0.45 + interestScore * 0.20 + intentionScore * 0.15 + profileStrength * 0.10 + freshness * 0.10);
      return { ...publicUser, compatibility: compat ? compat.percent : null, discoverScore };
    })
    .sort((a, b) => b.discoverScore - a.discoverScore || String(a.id).localeCompare(String(b.id)));
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .discover-card-actions .act-pass, .discover-card-actions .act-like { width:58px; height:58px; }
  @media (min-width:900px) { .discover-feed { gap:34px; } .discover-profile-card .swipe-card { max-width:760px; } }
  @media (max-width:700px) { .discover-feed { gap:20px; } .discover-profile-card .swipe-card { border-radius:18px; } .discover-card-actions { padding:14px 0 2px; } .discover-card-actions .act-pass, .discover-card-actions .act-like { width:54px; height:54px; } }
    ...
    <div class="content-grid">
      <div class="discover-col">
        ...
        <div class="view-panel visible" id="panel-discover">
          ...
          <div class="swipe-hint" id="swipe-hint">Scroll to discover more people</div>
        </div>
...
function renderSwipeCard() {
  ...
  hint.textContent = 'Scroll to discover more people';
  feed.innerHTML = state.candidates.map(c => \`
    <div class="discover-profile-card">
    <div class="swipe-card">
      ...
      <div class="discover-card-actions">
        <button class="act-pass" onclick="discoverAction('\${c.id}','pass',this)" aria-label="Pass" title="Pass"><svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M5 5l14 14M19 5 5 19"/></svg></button>
        <button class="act-like" onclick="discoverAction('\${c.id}','like',this)" aria-label="Like" title="Like"><svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor" stroke="none" aria-hidden="true" focusable="false"><path d="M12 20.5s-7.5-4.6-10-9.3C.4 8 1.7 4.6 5 3.6c2.2-.6 4.3.3 5.6 2.1l1.4 1.9 1.4-1.9C14.7 3.9 16.8 3 19 3.6c3.3 1 4.6 4.4 3 7.6-2.5 4.7-10 9.3-10 9.3Z"/></svg></button>
      </div>
      </div>
    </div>
    </div>
...
async function discoverAction(targetUserId, direction, button) {
  const c = state.candidates.find(x => x.id === targetUserId);
  if (!c || !['like', 'pass'].includes(direction)) return;
  if (button) button.disabled = true;
  try {
    const data = await api('/api/like', { method: 'POST', body: JSON.stringify({ targetUserId, direction }) });
    state.candidates = state.candidates.filter(x => x.id !== targetUserId);
    state.allCandidates = state.allCandidates.filter(x => x.id !== targetUserId);
    renderSwipeCard();
    if (data.matched) { await loadMatches(); alert("It's a match with " + c.name + "!"); }
  } catch (err) {
...
async function swipeAction(direction) {
  ...
  const card = document.querySelector('.discover-profile-card');
