...
async function handleDiscover(req, res, query) {
  ...
  // Discover is a scrollable recommendation feed. Explicit likes/passes are
  // excluded so users do not repeatedly see profiles they already acted on.
  ...
  function overlapScore(a, b) {
    ...
    const setB = new Set(bb.map(v => v.toLowerCase()));
    const matches = aa.filter(v => setB.has(v.toLowerCase())).length;
    return Math.min(100, Math.round((matches / Math.max(aa.length, bb.length)) * 100));
  }

  function questionnaireScore(a, b) {
    const qa = a || {};
    const qb = b || {};
    const keys = QUESTIONNAIRE.map(q => q.id);
    let total = 0;
    let matched = 0;
    for (const key of keys) {
      if (qa[key] === undefined || qb[key] === undefined) continue;
      total += 1;
      if (Array.isArray(qa[key]) || Array.isArray(qb[key])) {
        const av = Array.isArray(qa[key]) ? qa[key] : [];
        const bv = Array.isArray(qb[key]) ? qb[key] : [];
        const sb = new Set(bv);
        const intersection = av.filter(v => sb.has(v)).length;
        const union = new Set([...av, ...bv]).size;
        matched += union ? intersection / union : 1;
      } else if (qa[key] === qb[key]) {
        matched += 1;
      }
    }
  ...
  function activityScore(createdAt) {
    if (!createdAt) return 50;
    const ageDays = Math.max(0, (now - new Date(createdAt).getTime()) / 86400000);
    // Newer profiles get a modest freshness boost without overwhelming compatibility.
    return Math.max(20, Math.round(100 - Math.min(ageDays, 80)));
  }
  ...
  const candidates = (allUsers || [])
    ...
    .map(u => {
      ...
      const compatibility = compat ? compat.percent : questionnaireScore(user.questionnaire, u.questionnaire);
      const interestScore = overlapScore(user.tags, u.tags);
      const intentionScore = overlapScore(user.dating_intentions, u.dating_intentions);
      const profileStrength = computeProfileStrength(toPublicUser(u)).percentage;
      const freshness = activityScore(u.created_at);

      // Initial Discover ranking. These are recommendation weights, not a claim
      // about Bumble's private algorithm. Compatibility is deliberately dominant.
  ...
  // Sort into a recommendation feed. Add a tiny deterministic tie-breaker so
  // equal scores don't reshuffle on every request.
  candidates.sort((a, b) => {
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .discover-feed { display:flex; flex-direction:column; gap:28px; width:100%; }
  .discover-profile-card { width:100%; scroll-margin-top:24px; }
  .discover-profile-card .swipe-card { max-width:720px; margin:0 auto; }
  .discover-card-actions { display:flex; justify-content:center; align-items:center; gap:18px; padding:18px 0 4px; }
  .discover-card-actions button { border:none; border-radius:50%; display:flex; align-items:center; justify-content:center; box-shadow:0 6px 18px rgba(0,0,0,.08); cursor:pointer; font-family:inherit; }
  .discover-card-actions button:disabled { opacity:.55; cursor:wait; }
  .discover-card-actions .act-pass { width:58px; height:58px; font-size:30px; line-height:1; }
  .discover-card-actions .act-like { width:58px; height:58px; font-size:25px; line-height:1; }
  .discover-score-badge { z-index:3; }
  @media (min-width: 900px) {
    .discover-feed { gap:34px; }
    .discover-profile-card .swipe-card { max-width:760px; }
  }
  @media (max-width: 700px) {
    .discover-feed { gap:20px; }
    .discover-profile-card .swipe-card { border-radius:18px; }
    .discover-card-actions { padding:14px 0 2px; }
    .discover-card-actions .act-pass, .discover-card-actions .act-like { width:54px; height:54px; }
  }
    ...
    <div class="content-grid">
      <div class="discover-col">
        ...
        <div class="view-panel visible" id="panel-discover">
          ...
          <div id="discover-feed" class="discover-feed" aria-live="polite"></div>
          <div class="swipe-hint" id="swipe-hint">Scroll to discover more people</div>        </div>
...
function renderSwipeCard() {
  ...
  if (!state.candidates.length) {
    feed.innerHTML = '<div class="empty-state">No more profiles right now. Check back later!</div>';
    if (hint) hint.textContent = 'Check back later for more people.';
    return;
  }

  feed.innerHTML = state.candidates.map((c, index) => {
    ...
    const interests = (c.tags || []).map(t => '<span class="interest-chip">' + t + '</span>').join('');
    const vibes = (c.galleryUrls || []).map((url, i) => '<div class="swipe-vibes-thumb"><img src="' + url + '" alt="Vibes photo ' + (i + 1) + '" loading="lazy" /></div>').join('');
    const prompts = (c.prompts || []).filter(p => p && p.question && p.answer).map(p => '<div class="prompt-view-card"><div class="prompt-view-question">' + p.question + '</div><div class="prompt-view-answer">' + p.answer + '</div></div>').join('');
    const quiz = (c.quizAnswers || []).map(q => '<div class="prompt-view-card"><div class="prompt-view-question">' + q.question + '</div><div class="prompt-view-answer">' + q.answer + '</div></div>').join('');
    const likeId = 'discover-like-' + c.id;
    const passId = 'discover-pass-' + c.id;
    return \`
      <article class="discover-profile-card" data-profile-id="\${c.id}" data-index="\${index}">
        <div class="swipe-card">
          <div class="swipe-photo" style="\${photoStyle}">
            <div class="photo-tag">\${c.tags && c.tags.length ? c.tags[0] : 'New here'}</div>
            \${c.compatibility !== null && c.compatibility !== undefined ? \`<div class="photo-counter compat-badge discover-score-badge">✦ \${c.compatibility}% Match</div>\` : ''}
          ...
          <div class="swipe-card-body">
            ...
            \${c.datingIntentions && c.datingIntentions.length ? \`<div class="swipe-section"><div class="swipe-section-title"><span class="icon-circle">\${LOOKING_FOR_ICON}</span>I'm looking for</div><div class="about-pills-grid">\${buildLookingForPillsHTML(c)}</div></div>\` : ''}
            \${c.tags && c.tags.length ? \`<div class="swipe-section"><div class="swipe-section-title"><span class="icon-circle">✦</span>Interests</div><div class="interests-chips">\${interests}</div></div>\` : ''}
            \${c.galleryUrls && c.galleryUrls.length ? \`<div class="swipe-section"><div class="swipe-section-title"><span class="icon-circle">▣</span>My Vibes</div><div class="swipe-vibes-grid">\${vibes}</div></div>\` : ''}
            \${prompts ? \`<div class="swipe-section"><div class="swipe-section-title"><span class="icon-circle">☷</span>Prompts</div>\${prompts}</div>\` : ''}
            \${quiz ? \`<div class="swipe-section"><div class="swipe-section-title"><span class="icon-circle">✦</span>Compatibility Answers</div>\${quiz}</div>\` : ''}
          </div>
          <div class="discover-card-actions">
            <button id="\${passId}" class="act-pass" onclick="discoverAction('\${c.id}','pass', this)" aria-label="Pass \${c.name}" title="Pass">×</button>
            <button id="\${likeId}" class="act-like" onclick="discoverAction('\${c.id}','like', this)" aria-label="Like \${c.name}" title="Like">♥</button>
          </div>
        </div>
      </article>\`;
  }).join('');

  if (hint) hint.textContent = state.candidates.length > 1 ? 'Scroll to discover more people' : 'You reached the end of Discover.';
}

async function discoverAction(targetUserId, direction, button) {
  const candidate = state.candidates.find(c => c.id === targetUserId);
  if (!candidate || !button) return;
  const card = button.closest('.discover-profile-card');
  const buttons = card ? card.querySelectorAll('button') : [button];
  buttons.forEach(b => b.disabled = true);
  try {
    const data = await api('/api/like', { method: 'POST', body: JSON.stringify({ targetUserId, direction }) });
    state.candidates = state.candidates.filter(c => c.id !== targetUserId);
    state.allCandidates = state.allCandidates.filter(c => c.id !== targetUserId);
    renderSwipeCard();
    if (data.matched) {
      await loadMatches();
...
async function swipeAction(direction) {
  ...
  if (!c) return;
  const card = document.querySelector(\`.discover-profile-card[data-profile-id="\${c.id}"]\`);
  const button = card ? card.querySelector(direction === 'like' ? '.act-like' : '.act-pass') : null;
  return discoverAction(c.id, direction, button);
}
