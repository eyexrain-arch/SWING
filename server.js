...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .discover-category-tabs { overflow-x: auto; overflow-y: hidden; flex-wrap: nowrap; -webkit-overflow-scrolling: touch; scrollbar-width: none; padding-bottom: 2px; }
  .discover-category-tabs::-webkit-scrollbar { display: none; }
  .discover-category-tabs button { flex: 0 0 auto; white-space: nowrap; }
  ...
  /* Swipe right ("like") or left ("pass") to move through profiles, like
     a dating-app card stack — drag with mouse or touch, or use the
     buttons below the stack. */
  .discover-stack-wrap { display: flex; flex-direction: column; align-items: center; gap: 6px; }
  .discover-stack-nav-wrap { position: relative; width: 100%; max-width: min(420px, 100%); margin: 0 auto; }
  .discover-nav-btn {
  ...
  .stack-card {
    ...
    box-shadow: 0 10px 40px rgba(75,20,45,0.1);
  }
  .stack-card .swipe-card { width: 100%; max-width: none; margin: 0; height: 100%; overflow-y: auto; border-radius: 0; box-shadow: none; }
  .stack-card-grip {
    position: absolute; inset: 0; touch-action: pan-y; will-change: transform;
    cursor: grab; transition: transform 0.3s ease;
  }
  .stack-card-grip.dragging { cursor: grabbing; transition: none; }
  .stack-card-grip.flying-out { transition: transform 0.45s ease, opacity 0.4s ease; }
  .swipe-stamp {
    position: absolute; top: 24px; z-index: 5; padding: 6px 16px; border: 3px solid;
    border-radius: 9px; font-weight: 800; font-size: 24px; letter-spacing: 1.5px;
    text-transform: uppercase; opacity: 0; pointer-events: none;
  }
  .swipe-stamp-like { left: 18px; color: #4caf7a; border-color: #4caf7a; transform: rotate(-16deg); }
  .swipe-stamp-nope { right: 18px; color: #d63868; border-color: #d63868; transform: rotate(16deg); }
  .discover-stack-actions { display: flex; justify-content: center; align-items: center; gap: 18px; padding: 22px 0 6px; }
  ...
  .discover-like-section .discover-stack-inner { position: relative; width: 100%; max-width: min(420px, 100%); min-height: 620px; margin: 0 auto; }
  ...
  @media (prefers-reduced-motion: reduce) {
    ...
    .stack-card-grip { transition: none; }
  }
      ...
      <div class="discover-col">
        ...
        <div class="view-panel visible" id="panel-discover">
          <div class="filter-tabs discover-category-tabs" role="tablist" aria-label="Discover categories">
            <button class="active" type="button" role="tab" aria-selected="true" onclick="setDiscoverTab('maylike')">💕 Users You May Like</button>
            <button type="button" role="tab" aria-selected="false" onclick="setDiscoverTab('liked')">❤️ Profiles You Liked</button>
            <button type="button" role="tab" aria-selected="false" onclick="setDiscoverTab('nearby')">📍 Users Nearby</button>
            <button type="button" role="tab" aria-selected="false" onclick="setDiscoverTab('new')">🆕 New Users</button>
          </div>
          <div class="discover-stack-wrap">
            <div class="discover-stack-nav-wrap">
              <div id="discover-list" class="discover-stack"></div>
              <button type="button" class="discover-nav-btn discover-nav-prev" id="discover-nav-prev" onclick="peekDiscoverCard(-1)" aria-label="Previous profile" title="Previous profile" style="display:none;"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M15 5l-7 7 7 7"/></svg></button>
              <button type="button" class="discover-nav-btn discover-nav-next" id="discover-nav-next" onclick="peekDiscoverCard(1)" aria-label="Next profile" title="Next profile" style="display:none;"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M9 5l7 7-7 7"/></svg></button>
            </div>
            <div class="swipe-actions" id="discover-stack-actions" style="display:none;">
              <button type="button" class="act-pass" onclick="swipeTopCard('pass')" aria-label="Pass" title="Pass"><svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M5 5l14 14M19 5 5 19"/></svg></button>
              <button type="button" class="act-like" onclick="swipeTopCard('like')" aria-label="Like" title="Like"><svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor" stroke="none" aria-hidden="true" focusable="false"><path d="M12 20.5s-7.5-4.6-10-9.3C.4 8 1.7 4.6 5 3.6c2.2-.6 4.3.3 5.6 2.1l1.4 1.9 1.4-1.9C14.7 3.9 16.8 3 19 3.6c3.3 1 4.6 4.4 3 7.6-2.5 4.7-10 9.3-10 9.3Z"/></svg></button>
            </div>
            <div class="swipe-hint" id="discover-stack-hint" style="display:none;">Drag the card, or swipe left to pass / right to like — use the arrows to browse, Pass/Like to decide</div>
          </div>
        </div>
...
function createFreshState() {
  return {
    ...
    peekIndex: 0, // which candidate is currently shown on top of the "may like" stack, for left/right browsing without deciding
  };
}
...
function renderDiscoverSection(title, profiles, allowLike) {
  if (!profiles || !profiles.length) return '';
  return \`<section class="discover-section">
    <div class="discover-section-title">\${escapeHtml(title)} <span>\${profiles.length}</span></div>
...
function renderDiscoverList() {
  ...
  const showStackUi = tab === 'maylike' && state.candidates.length > 0;
  if (actions) actions.style.display = showStackUi ? '' : 'none';
  if (hint) hint.style.display = showStackUi ? '' : 'none';
  if (navPrev) navPrev.style.display = showStackUi ? '' : 'none';
  if (navNext) navNext.style.display = showStackUi ? '' : 'none';

  if (tab !== 'maylike') {
    const config = {
      liked: ['Profiles You Liked', state.likedProfiles, false],
      nearby: ['Users Nearby', state.nearbyProfiles, true],
      new: ['New Users — Last 48 Hours', state.newProfiles, true],
    }[tab];
    stack.innerHTML = config ? (renderDiscoverSection(config[0], config[1], config[2]) || '<div class="empty-state">No profiles in this section right now.</div>') : '';
    return;
  }
  ...
  // peekIndex lets the left/right buttons browse ahead in the queue without
  // recording a like/pass — clamp it in case the queue shrank underneath it.
  if (state.peekIndex > state.candidates.length - 1) state.peekIndex = state.candidates.length - 1;
  if (state.peekIndex < 0) state.peekIndex = 0;

  const visible = state.candidates.slice(state.peekIndex, state.peekIndex + 3);
  const swipeStack = visible.map((c, i) => {
    const isTop = i === 0;
    return \`
      <div class="stack-card-grip" data-id="\${c.id}" data-top="\${isTop ? 'true' : 'false'}"
           style="z-index:\${10 - i}; transform: translateY(\${i * 10}px) scale(\${1 - i * 0.035}); \${isTop ? '' : 'pointer-events:none;'}">
        <div class="stack-card">
          \${isTop ? '<div class="swipe-stamp swipe-stamp-like">Like</div><div class="swipe-stamp swipe-stamp-nope">Nope</div>' : ''}
          \${buildCandidateCardHTML(c)}
        </div>
      </div>\`;
  }).join('');

  stack.innerHTML = \`<section class="discover-section discover-like-section">
    <div class="discover-section-title">Users You May Like <span>\${state.candidates.length}</span></div>
    <div class="discover-stack-inner">\${swipeStack}</div>
  </section>\`;

  const topEl = stack.querySelector('.discover-like-section .stack-card-grip[data-top="true"]');
  if (topEl) attachCardDragHandlers(topEl);

  if (navPrev) navPrev.disabled = state.peekIndex <= 0;
  if (navNext) navNext.disabled = state.peekIndex >= state.candidates.length - 1;
}

// Moves the "may like" stack forward/back to preview a different profile
// without recording a like or pass. delta is -1 (previous) or 1 (next).
function peekDiscoverCard(delta) {
  if (state.swipeBusy) return;
  const maxIndex = state.candidates.length - 1;
  if (maxIndex < 0) return;
  const next = Math.min(Math.max(state.peekIndex + delta, 0), maxIndex);
  if (next === state.peekIndex) return;
  state.peekIndex = next;
  renderDiscoverList();
}

function setDiscoverTab(tab) {
  if (!['maylike', 'liked', 'nearby', 'new'].includes(tab)) return;
...
function attachCardDragHandlers(el) {
  ...
  function setStamps(ratio) {
    if (likeStamp) likeStamp.style.opacity = Math.max(0, Math.min(1, ratio));
    if (nopeStamp) nopeStamp.style.opacity = Math.max(0, Math.min(1, -ratio));
  }

  function onPointerDown(e) {
    if (state.swipeBusy || (e.pointerType === 'mouse' && e.button !== 0)) return;
    dragging = true;
  ...
  function onPointerUp() {
    ...
    if (Math.abs(dx) > SWIPE_THRESHOLD) {
      performSwipe(dx > 0 ? 'like' : 'pass', el);
    } else {
      el.style.transform = '';
      setStamps(0);
    }
...
// Triggered by the Pass/Like buttons below the stack.
function swipeTopCard(direction) {
  const stack = document.getElementById('discover-list');
  const topEl = stack && stack.querySelector('.stack-card-grip[data-top="true"]');
  performSwipe(direction, topEl);
}

// Flies the given card element off-screen in the swipe direction, then
// commits the like/pass. The flying card is reparented to <body> with a
// fixed position matching its on-screen spot, so its animation keeps
// playing even after the stack underneath re-renders with the next card.
function performSwipe(direction, el) {
  if (state.swipeBusy) return;
  ...
  discoverListAction(id, direction).finally(() => { state.swipeBusy = false; });
}
...
function openCandidateDetail(id) {
  ...
  document.getElementById('candidate-detail-footer').innerHTML = \`
    <button class="act-pass" onclick="discoverListAction('\${id}','pass'); closeCandidateDetail();" aria-label="Pass" title="Pass"><svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M5 5l14 14M19 5 5 19"/></svg></button>
