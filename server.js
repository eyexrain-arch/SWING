...
async function handleUpdateGpsLocation(req, res) {
  const user = await requireAuth(req, res); if (!user) return;
  const body = await readBody(req); const latitude = Number(body.latitude); const longitude = Number(body.longitude); const accuracy = Number(body.accuracy);
  if (!Number.isFinite(latitude) || latitude < -90 || latitude > 90 || !Number.isFinite(longitude) || longitude < -180 || longitude > 180) return sendJSON(res, 400, { error: 'Valid latitude and longitude are required' });
  const { error } = await supabase.from('users').update({ gps_latitude: latitude, gps_longitude: longitude, gps_accuracy_m: Number.isFinite(accuracy) && accuracy >= 0 ? accuracy : null, gps_updated_at: new Date().toISOString() }).eq('id', user.id);
  if (error) return sendJSON(res, 503, { error: 'GPS storage is not configured. Run gps-migration.sql in Supabase first.' });
  sendJSON(res, 200, { ok: true });
}
function haversineKm(lat1, lon1, lat2, lon2) { const r=Math.PI/180, dLat=(lat2-lat1)*r, dLon=(lon2-lon1)*r, a=Math.sin(dLat/2)**2+Math.cos(lat1*r)*Math.cos(lat2*r)*Math.sin(dLon/2)**2; return 6371*2*Math.atan2(Math.sqrt(a),Math.sqrt(1-a)); }
...
async function handleDiscover(req, res, query) {
  ...
  const { data: swiped, error: swipedErr } = await supabase
    .from('likes').select('to_user_id, direction').eq('from_user_id', user.id);
  ...
  const viewerLat = Number(user.gps_latitude), viewerLon = Number(user.gps_longitude);
  const hasViewerGps = Number.isFinite(viewerLat) && Number.isFinite(viewerLon);
  const nearbyProfiles = hasViewerGps ? eligibleUsers.filter(u => !swipedIds.has(u.id) && Number.isFinite(Number(u.gps_latitude)) && Number.isFinite(Number(u.gps_longitude))).map(u => ({ ...toCandidate(u), distanceKm: haversineKm(viewerLat, viewerLon, Number(u.gps_latitude), Number(u.gps_longitude)) })).filter(c => c.distanceKm <= 50).sort((a,b) => a.distanceKm-b.distanceKm).slice(0,20).map(c => ({ ...c, distanceKm: Math.round(c.distanceKm*10)/10 })) : [];
      ...
      <div class="discover-col">
        ...
        <div class="view-panel visible" id="panel-discover">
          <div class="filter-tabs discover-category-tabs" role="tablist" aria-label="Discover categories">
            <button class="active" type="button" role="tab" aria-selected="true" onclick="setDiscoverTab('liked')">❤️ Profiles You Liked</button>
            ...
            <button type="button" role="tab" aria-selected="false" onclick="setDiscoverTab('maylike')">💕 Users You May Like</button>
          </div>
...
async function logout() {
  ...
  state = { token: null, user: null, candidates: [], matches: [], admirers: [], activeChat: null, eventSource: null, aiChatHistory: [], aiVoiceRecognition: null, aiVoiceListening: false, registerInterests: {}, interestsDraft: {} };
...
async function syncGpsLocation() {
  ...
  await new Promise(resolve => navigator.geolocation.getCurrentPosition(async position => {
    try { await api('/api/profile/gps', { method: 'PUT', body: JSON.stringify({ latitude: position.coords.latitude, longitude: position.coords.longitude, accuracy: position.coords.accuracy }) }); } catch (e) {}
    resolve();
...
function renderDiscoverSection(title, profiles, allowLike) {
  ...
  return \`<section class="discover-section">
    <div class="discover-section-title">\${escapeHtml(title)} <span>\${list.length}</span></div>
    \${list.length ? \`<div>\${list.map(c => renderDiscoverSectionRow(c, allowLike)).join('')}</div>\` : '<div class="empty-state">No profiles here yet.</div>'}
  </section>\`;
}

function setDiscoverTab(tab) {
  const allowed = ['liked', 'nearby', 'new', 'maylike'];
  state.activeDiscoverTab = allowed.includes(tab) ? tab : 'liked';
  document.querySelectorAll('.discover-category-tabs button').forEach(btn => {
    const onclick = btn.getAttribute('onclick') || '';
    const active = onclick.includes(\`'\${state.activeDiscoverTab}'\`);
...
function renderDiscoverList() {
  ...
  const tab = state.activeDiscoverTab || 'liked';
  if (tab === 'liked') {
    stack.innerHTML = renderDiscoverSection('Profiles You Liked', state.likedProfiles, false);
    if (actions) actions.style.display = 'none'; if (hint) hint.style.display = 'none'; return;
  }
  if (tab === 'nearby') {
    stack.innerHTML = renderDiscoverSection('Users Nearby', state.nearbyProfiles, true);
    if (actions) actions.style.display = 'none'; if (hint) hint.style.display = 'none'; return;
  }
  if (tab === 'new') {
    stack.innerHTML = renderDiscoverSection('New Users — Last 48 Hours', state.newProfiles, true);
    if (actions) actions.style.display = 'none'; if (hint) hint.style.display = 'none'; return;
  }
  if (!state.candidates.length) {
    stack.innerHTML = \`<section class="discover-section discover-like-section"><div class="discover-section-title">Users You May Like <span>0</span></div><div class="empty-state">No more profiles right now. Check back later!</div></section>\`;
    if (actions) actions.style.display = 'none'; if (hint) hint.style.display = 'none'; return;
  }
  if (actions) actions.style.display = ''; if (hint) hint.style.display = '';
  const visible = state.candidates.slice(0, 3);
  const swipeStack = visible.map((c, i) => {
    const isTop = i === 0;
    return \`<div class="stack-card-grip" data-id="\${c.id}" data-top="\${isTop ? 'true' : 'false'}" style="z-index:\${10 - i}; transform: translateY(\${i * 10}px) scale(\${1 - i * 0.035}); \${isTop ? '' : 'pointer-events:none;'}"><div class="stack-card">\${isTop ? '<div class="swipe-stamp swipe-stamp-like">Like</div><div class="swipe-stamp swipe-stamp-nope">Nope</div>' : ''}\${buildCandidateCardHTML(c)}</div></div>\`;
  }).join('');
  stack.innerHTML = \`<section class="discover-section discover-like-section"><div class="discover-section-title">Users You May Like <span>\${state.candidates.length}</span></div><div class="discover-stack-inner">\${swipeStack}</div></section>\`;
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/profile/gps' && req.method === 'PUT') {
      return await handleUpdateGpsLocation(req, res);
    }
