...
// Whether `other` is someone `viewer` should be shown at all — used by
// both the real Discover feed and the AI matchmaker's candidate context so
// the two can't drift out of sync (see handleAiChat, which used to select
// a different set of columns and skip the discoverable/block checks
// entirely, so it could recommend someone who had opted out of Discover
// or blocked/been blocked by the viewer).
...
async function handleAiChat(req, res) {
  ...
  // the exact same eligibility rules as the real Discover feed (discoverable
  // flag, blocks, gender matching) via the shared isEligibleCandidate() —
  // this used to reimplement a slightly different, out-of-date copy of
  // those rules by hand, which meant Swing AI could recommend someone who
  // had opted out of Discover, or who'd blocked/been blocked by the viewer.
  const { data: swiped } = await supabase
    .from('likes').select('to_user_id').eq('from_user_id', user.id);
  const swipedIds = new Set((swiped || []).map(l => l.to_user_id));
  const blockedIds = await getBlockedIdSet(user.id);
  const { data: allUsers } = await supabase.from('users').select('id,name,age,bio,country,tags,gender,discoverable,interested_in');
...
function renderAiChat() {
  ...
  if (!state.aiChatHistory.length) {
    ...
    el.innerHTML = state.aiChatHistory.map(m => \`<div class="bubble \${m.role === 'user' ? 'mine' : 'theirs'}">\${escapeHtml(m.content)}</div>\`).join('');
  }
