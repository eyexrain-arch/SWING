...
async function handleUpdateGpsLocation(req, res) {
  ...
  if (!Number.isFinite(latitude) || latitude < -90 || latitude > 90 || !Number.isFinite(longitude) || longitude < -180 || longitude > 180) return sendJSON(res, 400, { error: 'Invalid GPS coordinates' });
  ...
  if (error) { if (String(error.message || '').toLowerCase().includes('gps_')) return sendJSON(res, 503, { error: 'Run gps-migration.sql in Supabase first.' }); throw error; }
  sendJSON(res, 200, { ok: true });
}

function haversineKm(lat1, lon1, lat2, lon2) { const r=Math.PI/180, a=Math.sin((lat2-lat1)*r/2)**2+Math.cos(lat1*r)*Math.cos(lat2*r)*Math.sin((lon2-lon1)*r/2)**2; return 6371*2*Math.atan2(Math.sqrt(a),Math.sqrt(1-a)); }
...
async function handleDiscover(req, res, query) {
  ...
  const userLat = Number(user.gps_latitude), userLon = Number(user.gps_longitude);
  const nearbyProfiles = Number.isFinite(userLat) && Number.isFinite(userLon)
    ? eligibleUsers.filter(u => !swipedIds.has(u.id)).map(u => { const lat=Number(u.gps_latitude), lon=Number(u.gps_longitude); if(!Number.isFinite(lat)||!Number.isFinite(lon)) return null; const d=haversineKm(userLat,userLon,lat,lon); return d<=50 ? {...toCandidate(u), distanceKm:Math.round(d*10)/10} : null; }).filter(Boolean).sort((a,b)=>a.distanceKm-b.distanceKm).slice(0,20)
    : [];
    ...
    <div class="content-grid">
      <div class="discover-col">
        ...
        <div class="view-panel visible" id="panel-discover">
          <div class="discover-heading">Discover</div>
...
async function syncGpsLocation() {
  ...
  await new Promise(resolve => navigator.geolocation.getCurrentPosition(async p => { try { await api('/api/profile/gps', {method:'PUT', body:JSON.stringify({latitude:p.coords.latitude, longitude:p.coords.longitude, accuracy:p.coords.accuracy})}); } catch(e) { console.warn('GPS save failed:', e.message); } resolve(); }, () => resolve(), {enableHighAccuracy:true, timeout:10000, maximumAge:300000}));
}
...
function renderDiscoverList() {
  ...
  const sections = [
    renderDiscoverSection('Profiles You Liked', state.likedProfiles, false),
    renderDiscoverSection('Users Nearby', state.nearbyProfiles, true),
    renderDiscoverSection('New Users — Last 48 Hours', state.newProfiles, true),
  ].join('');

  if (!state.candidates.length) {
    stack.innerHTML = sections || '<div class="empty-state">No more profiles right now. Check back later!</div>';
  ...
  stack.innerHTML = sections + \`<section class="discover-section discover-like-section">
    <div class="discover-section-title">Users You Can Like <span>\${state.candidates.length}</span></div>
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/profile/gps' && req.method === 'PUT') return await handleUpdateGpsLocation(req, res);
