...
function applySecurityHeaders(res) {
  ...
  res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'none'; form-action 'self';");
...
async function handleDiscover(req, res, query) {
  ...
  const { data: allUsers, error: usersErr } = await supabase.from('users').select('id,name,age,bio,location,country,gender,tags,photo_url,gallery_urls,created_at,languages,dating_intentions,prompts,questionnaire,verified');
  if (usersErr) throw usersErr;

  const countryFilter = (query.get('country') || '').trim().toLowerCase();

  const genderMatches = u => {
...
async function handleAiChat(req, res) {
  ...
  const { data: swiped } = await supabase
    .from('likes').select('to_user_id').eq('from_user_id', user.id);
  const swipedIds = new Set((swiped || []).map(l => l.to_user_id));
  const { data: allUsers } = await supabase.from('users').select('id,name,age,bio,country,tags');
