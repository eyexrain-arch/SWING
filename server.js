...
async function finishQuestionnaire() {
  try {
    ...
    // Non-fatal — they can fill this in later.
  }
  await closeQuestionnaireScreen();
}
...
function buildAboutPillsHTML(u) {
  ...
  if (u.political) items.push(['political', 'Political views', titleCaseWords(u.political)]);
  if (u.children) items.push(['children', 'Children', titleCaseWords(u.children)]);
  if (u.wantsChildren) items.push(['wantsChildren', 'Wants children', titleCaseWords(u.wantsChildren)]);
  if (u.gender) items.push(['gender', 'Gender', titleCaseWords(u.gender)]);
  if (!items.length) return '';
  return items.map(([key, question, value]) => \`
    <div class="about-answer-row">
      <span class="icon-circle">\${QUICK_INFO_ICONS[key] || ''}</span>
      <div class="about-answer-content">
        <div class="about-answer-question">\${question}</div>
        <div class="about-answer-value">\${value}</div>
      </div>
    </div>
  \`).join('');
}
