...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .discover-carousel-card {
    position: relative; flex: 0 0 min(380px, 84vw); scroll-snap-align: center;
    border-radius: 22px; overflow: hidden; box-shadow: 0 10px 40px rgba(75,20,45,0.1);
    height: 620px; cursor: pointer;
  }
  .discover-carousel-card .swipe-card { width: 100%; max-width: none; margin: 0; height: 100%; overflow-y: auto; border-radius: 0; box-shadow: none; }
  .carousel-like-btn {
...
function renderDiscoverList() {
  ...
  // Everyone in the queue scrolls left-to-right as a filmstrip. There's no
  // pass/reject step — scrolling past a card simply moves on to the next
  // one; tapping the heart on a card is the only decision to make.
  ...
  const cards = state.candidates.map(c => \`
      <div class="discover-carousel-card" data-id="\${c.id}" onclick="openCandidateDetail('\${c.id}')">
        <button type="button" class="carousel-like-btn" onclick="event.stopPropagation(); carouselLikeCandidate('\${c.id}')" aria-label="Like \${escapeHtml(c.name)}" title="Like">\${heartIcon}</button>
