...
function renderDiscoverList() {
  ...
  if (!state.candidates.length) {
    list.innerHTML = '<div class="empty-state">No more profiles right now. Check back later!</div>';
    return;
  }
  list.innerHTML = state.candidates.map(c => {
    const photoStyle = c.photoUrl
      ? \`background-image:url('\${c.photoUrl}');\`
      : \`background:linear-gradient(135deg, \${colorFor(c.name)}, #2a1620);\`;
    const tags = (c.tags || []).slice(0, 4).map(t => '<span>' + escapeHtml(t) + '</span>').join('');
    return \`
      <div class="discover-row" onclick="openCandidateDetail('\${c.id}')">
        <div class="discover-row-photo" style="\${photoStyle}"></div>
        <div class="discover-row-body">
          <div class="discover-row-name">\${escapeHtml(c.name)}\${c.age ? ', ' + c.age : ''}</div>
          \${c.location ? '<div class="discover-row-loc"><svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M12 21s7-6.5 7-11.5A7 7 0 1 0 5 9.5C5 14.5 12 21 12 21Z"/><circle cx="12" cy="9.5" r="2.2"/></svg> ' + escapeHtml(c.location) + '</div>' : ''}
          \${c.bio ? '<div class="discover-row-bio">' + escapeHtml(c.bio) + '</div>' : ''}
          \${tags ? '<div class="discover-row-tags">' + tags + '</div>' : ''}
        </div>
        <button class="discover-row-like" onclick="event.stopPropagation(); discoverListAction('\${c.id}','like')" aria-label="Like \${escapeHtml(c.name)}" title="Like">
          <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor" stroke="none" aria-hidden="true" focusable="false"><path d="M12 20.5s-7.5-4.6-10-9.3C.4 8 1.7 4.6 5 3.6c2.2-.6 4.3.3 5.6 2.1l1.4 1.9 1.4-1.9C14.7 3.9 16.8 3 19 3.6c3.3 1 4.6 4.4 3 7.6-2.5 4.7-10 9.3-10 9.3Z"/></svg>
        </button>
        <button class="discover-row-kebab" onclick="event.stopPropagation(); toggleDiscoverRowMenu(event,'\${c.id}')" aria-label="More options" title="More options">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" stroke="none" aria-hidden="true" focusable="false"><circle cx="5" cy="12" r="1.8"/><circle cx="12" cy="12" r="1.8"/><circle cx="19" cy="12" r="1.8"/></svg>
        </button>
        <div class="discover-row-menu" id="row-menu-\${c.id}">
          <button type="button" onclick="event.stopPropagation(); openCandidateDetail('\${c.id}'); closeAllDiscoverMenus();">View profile</button>
          <button type="button" onclick="event.stopPropagation(); discoverListAction('\${c.id}','pass');">Not interested</button>
        </div>
      </div>
    \`;
  }).join('');
