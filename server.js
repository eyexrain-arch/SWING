...
async function handleUploadPhoto(req, res) {
  ...
  // Remove any previous photo file for this user
  if (user.photoUrl) {
...
function serveUploadedFile(req, res, filename) {
  ...
  res.writeHead(200, { 'Content-Type': contentType, 'Content-Length': data.length, 'Cache-Control': 'public, max-age=31536000' });
  res.end(data);
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (uploadMatch && req.method === 'GET') {
      return serveUploadedFile(req, res, uploadMatch[1]);
    }
