...
// Escape untrusted values before placing them into HTML. User profile text,
// prompt answers, and AI responses must never be treated as markup.
...
// REQUIRED Supabase hardening (run once in SQL Editor):
//   ALTER TABLE users ADD COLUMN IF NOT EXISTS gallery_urls jsonb NOT NULL DEFAULT '[]';
//   ALTER TABLE users ADD COLUMN IF NOT EXISTS prompts jsonb NOT NULL DEFAULT '[]';
//   ALTER TABLE users ADD CONSTRAINT gallery_urls_max_3 CHECK (jsonb_array_length(gallery_urls) <= 3);
//   CREATE UNIQUE INDEX IF NOT EXISTS matches_key_unique ON matches(key);
//   CREATE UNIQUE INDEX IF NOT EXISTS likes_from_to_unique ON likes(from_user_id, to_user_id);
// ---------- Photo upload ----------
  ...
  if (direction === 'like') {
    ...
    if (reciprocal) {
      ...
      if (!existingMatch) {
        ...
        if (matchInsertErr) {
          // A unique constraint on matches.key makes concurrent reciprocal likes safe.
          // PostgreSQL duplicate-key errors mean another request created the match first.
          if (matchInsertErr.code !== '23505') throw matchInsertErr;
        }
        matched = true;
...
function avatarStyleAttr(u) {
  if (u && u.photoUrl) return \`background-image:url('\${escapeAttr(u.photoUrl)}');background-size:cover;background-position:center;color:transparent;\`;
...
function buildCandidateCardHTML(c) {
  const photoStyle = c.photoUrl
    ? \`background-image:url('\${escapeAttr(c.photoUrl)}');background-size:cover;background-position:center;\`
    ...
    <div class="swipe-card">
      <div class="swipe-photo" style="\${photoStyle}">
        ...
        <div class="swipe-details">
          <div class="name-row">\${escapeHtml(c.name)}\${c.age ? ', ' + escapeHtml(c.age) : ''}</div>
      ...
      <div class="swipe-card-body">
        <div class="swipe-section">
          ...
          <p class="swipe-section-text \${c.bio ? '' : 'empty'}">\${escapeHtml(c.bio || 'No bio yet.')}</p>
        ...
        <div class="swipe-section">
          ...
          <div class="swipe-vibes-grid">\${c.galleryUrls.map((url, i) => '<div class="swipe-vibes-thumb"><img src="' + escapeAttr(url) + '" alt="Vibes photo ' + (i + 1) + '" loading="lazy" /></div>').join('')}</div>
        </div>\` : ''}
...
function renderDiscoverList() {
  ...
  list.innerHTML = state.candidates.map(c => {
    const photoStyle = c.photoUrl
      ? \`background-image:url('\${escapeAttr(c.photoUrl)}');\`
      : \`background:linear-gradient(135deg, \${colorFor(c.name)}, #2a1620);\`;
    const tags = (c.tags || []).slice(0, 4).map(t => '<span>' + escapeHtml(t) + '</span>').join('');
    return \`
      <div class="discover-row" onclick="openCandidateDetail(\${JSON.stringify(String(c.id))})">
        <div class="discover-row-photo" style="\${photoStyle}"></div>
        <div class="discover-row-body">
          <div class="discover-row-name">\${escapeHtml(c.name)}\${c.age ? ', ' + escapeHtml(c.age) : ''}</div>
        ...
        <button class="discover-row-like" onclick="event.stopPropagation(); discoverListAction(\${JSON.stringify(String(c.id))},'like')" aria-label="Like \${escapeAttr(c.name)}" title="Like">
        ...
        <button class="discover-row-kebab" onclick="event.stopPropagation(); toggleDiscoverRowMenu(event,\${JSON.stringify(String(c.id))})" aria-label="More options" title="More options">
        ...
        <div class="discover-row-menu" id="row-menu-\${escapeAttr(c.id)}">
          <button type="button" onclick="event.stopPropagation(); openCandidateDetail(\${JSON.stringify(String(c.id))}); closeAllDiscoverMenus();">View profile</button>
          <button type="button" onclick="event.stopPropagation(); discoverListAction(\${JSON.stringify(String(c.id))},'pass');">Not interested</button>
        </div>
      </div>
    \`;
  }).join('');
  ...
  el.innerHTML = state.matches.map(m => \`
    <div class="msg-item" style="background:white;border-radius:14px;padding:12px 14px;margin-bottom:10px;border:none;" onclick="openChat('\${m.matchKey}')">
      ...
      <div class="msg-body">
        ...
        <div class="msg-preview"><span class="text">\${escapeHtml(m.lastMessage ? m.lastMessage.text : 'Say hello')}</span>\${m.unread ? '<span class="unread-dot">' + m.unread + '</span>' : ''}</div>
      </div>
    </div>
  \`).join('');
}
...
function renderAdmirers() {
  ...
  el.innerHTML = state.admirers.map(a => \`
    <div class="admirer-card">
      ...
      <div class="aname">\${escapeHtml(a.name)}\${a.age ? ', ' + escapeHtml(a.age) : ''}</div>
...
function buildAboutPillsHTML(u) {
  ...
  return items.map(([key, value]) => \`
    <span class="about-pill"><span class="icon-circle">\${QUICK_INFO_ICONS[key] || ''}</span>\${escapeHtml(value)}</span>
  \`).join('');
}
...
function buildLookingForPillsHTML(u) {
  ...
  return values.map(v => \`
    <span class="about-pill"><span class="icon-circle">\${LOOKING_FOR_ICON}</span>\${escapeHtml(LOOKING_FOR_LABELS[v] || titleCaseWords(v))}</span>
  \`).join('');
}
    ...
    if (url) {
      html += \`
        <div class="vibes-slot">
          <img src="\${escapeAttr(url)}" alt="My Vibes photo \${i + 1}" loading="lazy" onload="this.classList.add('loaded')" />
