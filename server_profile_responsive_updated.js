...
// Fields counted toward the profile-completion percentage shown on the
// person's own Profile tab. Keep in sync with the "complete your profile"
// screen and with matchesFilters() on the frontend.
const PROFILE_COMPLETION_FIELDS = [
  'photoUrl', 'bio', 'location', 'country', 'gender', 'age', 'tags',
  'languages', 'education', 'heightCm', 'exercise', 'drinking', 'smoking',
  'religion', 'political', 'children', 'wantsChildren', 'datingIntentions',
];
...
function toOwnUser(row) {
  ...
  return { ...pub, questionnaire: row.questionnaire || {}, profileCompletion: computeProfileCompletion(pub) };
}
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Keep the profile compact on phones while giving tablets/desktops a
     balanced two-column layout. This only affects the Profile view. */
  @media (min-width: 721px) {
    ...
    .profile-view > .about-card,
    .profile-view > .interests-card,
    .profile-view > .quick-info-card,
    .profile-view > .vibes-card,
    .profile-view > .compatibility-card {
      min-width: 0;
    }
    .profile-form {
      width: min(100%, 760px);
  ...
  @media (min-width: 1100px) {
    ...
    .profile-header-card .profile-completion {
      grid-column: 2;
      margin: 14px 0 0;
    }
    .profile-edit-btn {
...
function updateProfileCompletionDisplay() {
  ...
  const pct = (typeof state.user.profileCompletion === 'number') ? state.user.profileCompletion : computeCompletionClient(state.user);
